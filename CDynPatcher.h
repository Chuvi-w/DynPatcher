...
public:
   ...
   std::vector<uint32_t> FindString(uint32_t StartAddr, const char* str, bool FullMatch = true);
   ...
   std::vector<uint32_t> FindDataRef(uint32_t StartAddr, const std::vector<uint32_t> &RefVec);
   std::vector<uint32_t> FindRef_Mov(uint32_t StartAddr, const std::vector<uint32_t> &RefVec);
   std::vector<uint32_t> FindRef_Push(uint32_t StartAddr, const std::vector<uint32_t> &RefVec);
   std::vector<uint32_t> FindRef_Call(uint32_t StartAddr, const std::vector<uint32_t> &RefVec);
   std::vector<uint32_t> FindRef_Jmp(uint32_t StartAddr, const std::vector<uint32_t> &RefVec);
   ...
   iBool    AddMemChangeInfo(void *Addr, uint32_t OriginalValue, void *OriginalFuncAddr = NULL);
   uint32_t HookImportFunc(const char *FuncName, void *NewAddr, const char *Library = NULL);
   uint32_t GetOriginalPtrValue(void *addr);
   uint32_t GetImportFuncAddr(const char *FuncName,const char *Library = NULL);
   ...
   const char *GetLibraryName() {return CBaseNotification::GetFileName(szLibName); }
   iBool IsValidCodeAddr(void *Addr);
   iBool IsRangeInLib(uint32_t Addr, uint32_t Size);
   iBool IsRangeInCode(uint32_t Addr, uint32_t Size)  {iBool ret=Code.IsRangeInSections(Addr, Size); return ret;}
   iBool IsRangeInData(uint32_t Addr, uint32_t Size) {iBool ret=Data.IsRangeInSections(Addr, Size); return ret;}
   iBool ContainsAddress(uint32_t Addr)               { auto ret = (Addr >= reinterpret_cast<uint32_t>(this->DllBase)) && (Addr < (reinterpret_cast<uint32_t>(this->DllBase) + this->DllSize)); return static_cast<iBool>(ret); }
   iBool ContainsAddress(void* Addr)                  { iBool ret = ContainsAddress(reinterpret_cast<uint32_t>(Addr)); return ret; }
...
public:
   ...
   template <typename T> iBool FindSymbol(const char* sName, T* pSym)
   {
      if (!DllBase)
      {
         return bFalse;
      ...
      auto GetProcAdr = [this](const char *Sym)->FARPROC
      {
         if (bLoadFromMemory)
         {
            return myGetProcAddress_LoadDLLInfo(&DllMemInfo, Sym);
         }
         else
         {
   ...
   template<typename T> int32_t GetVFuncOffset(T Func)
   {
      ...
      do
      {
         if (*reinterpret_cast<unsigned char*>(vAddr) == 0xE9)
         {
            ...
            vAddr = reinterpret_cast<void*>(reinterpret_cast<uint32_t>(vAddr)+*reinterpret_cast<int32_t*>(reinterpret_cast<uint32_t>(vAddr)+1) + 5);
         }
         else if (*reinterpret_cast<uint32_t*>(vAddr) == 0x60ff018b)
         {
            //8B 01               mov     eax, [ecx]
            //FF 60 XX            jmp     dword ptr[eax + XX]
            Offset = static_cast<int32_t>(*reinterpret_cast<unsigned char*>(reinterpret_cast<uint32_t>(vAddr)+4));
         }
         else if (*reinterpret_cast<uint32_t*>(vAddr) == 0xa0ff018b)
         {
            //8B 01               mov     eax, [ecx]
            //FF A0 XX XX XX XX   jmp     dword ptr[eax + XXXXXXXX]
            Offset = *reinterpret_cast<int32_t*>(reinterpret_cast<uint32_t>(vAddr)+4);
         }
	...
	template<typename F, typename T> F CastFunc(T Func)
	{
		void *Addr = reinterpret_cast<void*>((size_t &)Func);
		return (F&)(Addr);
	}
   template<typename F, typename T> F HookVFunctionCall(void **VTable, F OrigFunc, T NewAddr)
   {
      ...
      {
         DynErr( "Invalid offset %i in virtual table %p. Max offset=%i", Offset, VTable, MaxOffset - 1);
         return 0;
      }
      void *OrigAddr = reinterpret_cast<void*>(ChangePtrValue(&VTable[Offset], reinterpret_cast<void*>((size_t &)NewAddr)));
      if (!OrigAddr)
      {
...
template<typename T> struct ParseDescription_s
{
   T ParseFunc;
   const char *Name;
};
