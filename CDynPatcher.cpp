...
CDynPatcher::CDynPatcher() :DllBase(0), bSelfLoaded(bFalse), szLibName(0), CBaseNotification("CDynPatcher")
#ifdef POSIX
, DLHandler(0)
#endif
{
...
void* CDynPatcher::LocateLib(const char* LibName, uint32_t *pLibSize)
{
   ...
   auto NTHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>((size_t)dosHeader + dosHeader->e_lfanew);
   ...
   if (fp)
   {
      while (!feof(fp))
      {
         ...
         sscanf(buffer, "%lx-%lx", reinterpret_cast<uint32_t*> (&start), reinterpret_cast<uint32_t*> (&end));
         ...
         if (strcmp(LibName, DlLibPath) == 0)
         {
            ...
            if (pLibSize)
            {
               ...
               while (!feof(fp))
               {
                  ...
                  sscanf(buffer, "%lx-%lx %*s %*s %*s %d", reinterpret_cast<long unsigned int *> (&start), reinterpret_cast<long unsigned int *> (&end), &value);
                  ...
                  if (!value)
                  {
                     *pLibSize = AddrEnd - AddrStart;
							...
							{
								delete [] szLibName;
								szLibName=new char [strlen(DlInfo.dli_fname)+1];
                  ...
                  else
                  {
                     ...
                     //*pLibSize += (unsigned long)end - (unsigned long)start;
                  }
               }
            }
...
uint32_t CDynPatcher::GetMemoryFlags(void *addr)
{
   ...
   if (fp)
   {
      while (!feof(fp))
      {
         ...
         if (reinterpret_cast<uint32_t>(addr) >= Start&&reinterpret_cast<uint32_t>(addr) <= End)
         {
            if (sFlags[0] == 'r')
            {
               Flags |= PROT_READ;
            ...
            //DynMsg("Region=[%x->%p->%x](%s) [%x]", Start, addr, End, sFlags, Flags);
            break;
...
uint32_t CDynPatcher::MProtect(void *addr, uint32_t nBytes,uint32_t NewFlags)
{
   ...
   if (VirtualProtect(addr, nBytes, NewFlags, reinterpret_cast<PDWORD>(&OldProtect)))
   {
      return OldProtect;
   }
#else
   ...
   void *paddr = reinterpret_cast<void*>((reinterpret_cast<size_t>(addr)) & ~(PAGESIZE - 1));
   //DynMsg("pid=%i,addr=%p, paddr=%p,nPages=%i,OldProtect=%i",getpid(),addr,paddr,nPages,OldProtect);
...
iBool CDynPatcher::CloseLib()
{
   ...
   {
#ifndef _WIN32
      if(DLHandler){ dlclose(DLHandler);DLHandler=NULL;}
#else
      if (DllBase)
      {
...
iBool CDynPatcher::ParseGenericDllData_PE(void* FileData, uint32_t FileSize)
{
   ...
   //DynMsg("Base addr=%p\n", this->DllBase);
   if (FileSize < sizeof(IMAGE_DOS_HEADER) + sizeof(IMAGE_NT_HEADERS) + sizeof(IMAGE_OPTIONAL_HEADER) + sizeof(IMAGE_SECTION_HEADER))
   {
   ...
   PIMAGE_NT_HEADERS NTHeaders = (PIMAGE_NT_HEADERS)((size_t)FileData + dosHeader->e_lfanew);
   if (NTHeaders->Signature != IMAGE_NT_SIGNATURE)
   {
   ...
   for (i = 0; i < NTHeaders->FileHeader.NumberOfSections; i++, cSection++)
   {
      ...
      {
         if (cSection->Characteristics&IMAGE_SCN_MEM_EXECUTE&&cSection->Characteristics&IMAGE_SCN_MEM_READ)
         {
            Code.Add((uint32_t)cSection->VirtualAddress, cSection->Misc.VirtualSize, bTrue, cSection->Characteristics&IMAGE_SCN_MEM_WRITE ? bTrue : bFalse, bTrue, this, SectionName);
         }
         else
         {
            DynMsg("Non-executable or not-readable section '%s' with code!", SectionName);
         }
         
      }
      else if (cSection->Characteristics&IMAGE_SCN_CNT_INITIALIZED_DATA || cSection->Characteristics&IMAGE_SCN_CNT_UNINITIALIZED_DATA)
      {
         Data.Add((uint32_t)cSection->VirtualAddress, cSection->Misc.VirtualSize, bTrue, cSection->Characteristics&IMAGE_SCN_MEM_WRITE ? bTrue : bFalse, bFalse, this, SectionName);
      }
      else
      {
         //Data.Add((uint32_t)this->DllHandler + cSection->VirtualAddress, cSection->Misc.VirtualSize, this, SectionName);
      }
   }
	if (!Code) 
   {
   ...
   //DynMsg("ENTRY_IMPORT.size=%i", NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size);
	//DynMsg("ENTRY_IAT.size=%i", NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size);
   //DynMsg("ENTRY_BOUND_IMPORT.size=%i", NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT].Size);
   //DynMsg("ENTRY_DELAY_IMPORT.size=%i", NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].Size);
   //DynMsg("ENTRY_BASERELOC.size=%i", NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].Size);
   ...
   if (NTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size != 0)/*if size of the table is 0 - Import Table does not exist */
   {
      ...
      pImportDescriptor = reinterpret_cast<PIMAGE_IMPORT_DESCRIPTOR>(reinterpret_cast<DWORD>(DllBase)+OptionalHeader->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);

      while (pImportDescriptor->Name)
      {
         DllName = reinterpret_cast<char*>(reinterpret_cast<DWORD>(DllBase)+pImportDescriptor->Name);

         if (DllName)
         {
            pThunkDataName = reinterpret_cast<PIMAGE_THUNK_DATA>(reinterpret_cast<DWORD>(DllBase)+pImportDescriptor->OriginalFirstThunk);
            pThunkDataFunc = reinterpret_cast<PIMAGE_THUNK_DATA>(reinterpret_cast<DWORD>(DllBase)+pImportDescriptor->FirstThunk);
            while (pThunkDataName->u1.AddressOfData)
            {
               iFuncName = NULL;
               if (!IMAGE_SNAP_BY_ORDINAL(pThunkDataName->u1.Ordinal))
               {
                  iFuncName = reinterpret_cast<char*>(reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(reinterpret_cast<DWORD>(DllBase)+pThunkDataName->u1.ForwarderString)->Name);
                  ImpFuncOrd = NULL;
               }
               else
               {
               ...
               tImpDescr->pImpFuncAddr = reinterpret_cast<uint32_t*>(&pThunkDataFunc->u1.Function);
            ...
            szTmpName[0] = 0;
            MEMORY_BASIC_INFORMATION mem;
            VirtualQuery(reinterpret_cast<void*>(tImpDescr->ImpFuncAddr), &mem, sizeof(mem));
            GetModuleFileNameA(reinterpret_cast<HMODULE>(mem.AllocationBase), szTmpName, sizeof(szTmpName) - 1);
...
iBool CDynPatcher::ParseGenericDllData_ELF(void* FileData, uint32_t FileSize) 
{
   ...
   auto GetSectionHdr = [ehdr,FileData](uint32_t SectID)->Elf32_Shdr*
   {
      ...
      return  reinterpret_cast<Elf32_Shdr*>(reinterpret_cast<size_t>(FileData)+ehdr->e_shoff + ehdr->e_shentsize*SectID);
   };
...
//   FILE *Dump;
   for (i = 0; i < ehdr->e_shnum; i++) 
   {
      ...
      sname = StringTable + csHdr->sh_name;
		//DynMsg("Section %s:%p-%p\n",sname,(uint32_t)csHdr->sh_addr,(uint32_t)csHdr->sh_addr+csHdr->sh_size);
      if (csHdr->sh_type&SHT_PROGBITS)
      {
         if (csHdr->sh_flags&SHF_EXECINSTR)
         {
            Code.Add((uint32_t)csHdr->sh_addr, csHdr->sh_size, bTrue, csHdr->sh_flags&SHF_WRITE ? bTrue : bFalse, bTrue, this, sname);
         }
         else //if(!strcmp(sname,".data")||!strcmp(sname,".rodata"))
         {
            Data.Add((uint32_t)csHdr->sh_addr, csHdr->sh_size, bTrue, csHdr->sh_flags&SHF_WRITE ? bTrue : bFalse, bFalse, this, sname);
         }
      }

      if (!strcmp(sname, ".rel.plt"))
      {
         rel_plt_table = reinterpret_cast<Elf32_Rel*>(reinterpret_cast<size_t>(DllBase)+csHdr->sh_addr);
         rel_plt_amount = csHdr->sh_size / sizeof(Elf32_Rel);
         DynWarn("rel_plt_table =%p, size=%x", rel_plt_table, rel_plt_amount);
      }
      if (!strcmp(sname, ".rel.dyn"))
      {
         rel_dyn_table = reinterpret_cast<Elf32_Rel*>(reinterpret_cast<size_t>(DllBase)+csHdr->sh_addr);
         rel_dyn_amount = csHdr->sh_size / sizeof(Elf32_Rel);
         DynWarn("rel_dyn_table =%p, size=%x", rel_dyn_table, rel_dyn_amount);
      }
      if (csHdr->sh_type==SHT_DYNSYM)
      {
         dyn_sym = reinterpret_cast<Elf32_Sym *>(reinterpret_cast<size_t>(DllBase)+csHdr->sh_addr);
         dyn_sym_amount = csHdr->sh_size / sizeof(Elf32_Sym);
         if (csHdr->sh_link&&csHdr->sh_link<ehdr->e_shnum)
         {
            dyn_strings = reinterpret_cast<const char*>(reinterpret_cast<uint32_t>(DllBase)+GetSectionHdr(csHdr->sh_link)->sh_addr);
      ...
      if(sname)
      {
         DynMsg("section %s, type=%i,sz=%i",sname,csHdr->sh_type,csHdr->sh_size);
         sprintf(fname,"/home/chuvi/%s_sect_%s.bin",szLibName,sname+1);
         Dump=fopen(fname,"wb");
         if(Dump)
         {
            fwrite((void*)((uint32_t)(this->DllBase)+(uint32_t)csHdr->sh_addr),csHdr->sh_size,1,Dump);
            fclose(Dump);
         }
...
//   ImpDynType_e ImpType;
   auto CheckFuncPtr=[this](uint32_t FuncAddr,const char *FuncName,const char **LibName)->iBool
   {
      ...
      if (dladdr(reinterpret_cast<void*>(FuncAddr), &DlInfo))
      {
         if(!DlInfo.dli_saddr)
         {
            if(!DlInfo.dli_fname)
            {
               DynWarn("Unable to get lib for \"%s\" %p",FuncName,FuncAddr);
            }
            else
            {
               ...
               {
                  DlFunc=reinterpret_cast<uint32_t>(dlsym(DlLib,const_cast<char*>(FuncName)));
                  if(!DlFunc)
                  {
                     DynWarn("Unable to find \"%s\" in \"%s\"",FuncName,DlInfo.dli_fname);
                  }
                  dlclose(DlLib);
               }
...
*/      
         if(DlFunc!=FuncAddr)
         {
            DynWarn("\"%s\" addr error (%p!=%p)",FuncName,DlFunc,FuncAddr);
         }
         else
         {
            ...
            if (LibName)
            {
               *LibName = DlInfo.dli_fname ? GetFileName(DlInfo.dli_fname) : 0;
            }
            return bTrue;
         }
      }
   ...
   auto AddImportFunc=[this](const char *FunctionName,const char *Library,uint32_t Addr, ImpDynType_e ImpType)
   {
      ...
      {
         if(!strcmp(Im->FunctionName,FunctionName)&&!strcmp(Im->LibraryName,Library))
         {
            if(ImpType==IDT_DIRECT)
            {
               //DynWarn("%s direct sz=%i",FunctionName,Im->DirectPtrs.size())
               ...
               Im->DirectPtrs.push_back(reinterpret_cast<uint32_t*>(Addr));
      ...
      switch(ImpType)
      {
            ...
            case IDT_DIRECT:
            {
               ...
               ImpDescr->DirectPtrs.push_back(reinterpret_cast<uint32_t*>(Addr));
   ...
   if (dyn_strings)
   {
      ...
      for (size_t j = 0; j < rel_plt_amount; ++j)
      {
         ...
         if (CurSym->st_value||!CurSym->st_name||ELF32_ST_TYPE(CurSym->st_info)!=STT_FUNC)
         {
            continue;
         }
         ...
         FuncAddr = reinterpret_cast<uint32_t>(DllBase)+rel_plt_table[j].r_offset;
         if (CheckFuncPtr(*reinterpret_cast<uint32_t*>(FuncAddr), CurFuncName, &CurLibName))
         {   
            AddImportFunc(CurFuncName, CurLibName,FuncAddr, IDT_NO);
            //DynWarn("rel_plt(%i):[%05i:%s]n=%08x,v=%08x,sz=%04x,bi=%i,ti=%i,o=%i,ndx=%i,fa=%p,rt=%i",j,DynSymID,CurFuncName,CurSym->st_name,CurSym->st_value,CurSym->st_size, ELF32_ST_BIND(CurSym->st_info),ELF32_ST_TYPE(CurSym->st_info),CurSym->st_other,CurSym->st_shndx,FuncAddr,DynSymRtType);
         }
     }
      ...
      for (size_t j = 0; j < rel_dyn_amount; ++j)
      {
         ...
         switch(DynSymRtType)
         {
            ...
            case R_386_PC32:
            {
               CallAddr=reinterpret_cast<uint32_t*>(reinterpret_cast<uint32_t>(DllBase)+rel_dyn_table[j].r_offset);
               FuncAddr=(*CallAddr + reinterpret_cast<uint32_t>(CallAddr) + sizeof(uint32_t));
               if(CheckFuncPtr(FuncAddr,CurFuncName,&CurLibName))
               {
                  AddImportFunc(CurFuncName, CurLibName,FuncAddr, IDT_RELATIVE);
               }
               break;
            }
            case R_386_32:
            {
               ...
               FuncAddr=(reinterpret_cast<uint32_t>(DllBase)+rel_dyn_table[j].r_offset);
               //if(CheckFuncPtr(*reinterpret_cast<uint32_t*>(FuncAddr),CurFuncName))
               if (CheckFuncPtr(*reinterpret_cast<uint32_t*>(FuncAddr), CurFuncName, &CurLibName))
               {
                  AddImportFunc(CurFuncName,CurLibName,FuncAddr,IDT_DIRECT);
               }
                
               break;
            }
         }
         uint32_t *p=reinterpret_cast<uint32_t*>(reinterpret_cast<uint32_t>(DllBase)+rel_dyn_table[j].r_offset);
...
std::vector<uint32_t> CDynPatcher::FindString(uint32_t StartAddr, const char* str, bool FullMatch /*= true*/)
{
   auto ret = Data.FindString(str, StartAddr, FullMatch);
   return ret;
}

std::vector<uint32_t> CDynPatcher::FindDataRef(uint32_t StartAddr, uint32_t RefAddr)
{
   auto ret = Code.FindDataRef(RefAddr, StartAddr);
   return ret;
}

std::vector<uint32_t> CDynPatcher::FindRef_Mov(uint32_t gStartAddr, uint32_t RefAddress)
{
   auto ret = Code.FindRef_Mov(gStartAddr, RefAddress);
   return ret;
}

std::vector<uint32_t> CDynPatcher::FindRef_Push(uint32_t StartAddr, uint32_t RefAddress)
{
   auto ret = Code.FindRef_Push(StartAddr, RefAddress);
	return ret;
}

std::vector<uint32_t> CDynPatcher::FindRef_Call(uint32_t StartAddr, uint32_t RefAddress)
{
...
std::vector<uint32_t> CDynPatcher::FindRef_Jmp(uint32_t StartAddr, uint32_t RefAddress)
{
   auto ret = Code.FindRef_Jmp(StartAddr, RefAddress);
	return ret;
}

std::vector<uint32_t> CDynPatcher::FindDataRef(uint32_t StartAddr, const std::vector<uint32_t> &RefVec)
{
...
std::vector<uint32_t> CDynPatcher::FindRef_Mov(uint32_t StartAddr, const std::vector<uint32_t> &RefVec)
{
   std::vector<uint32_t> Refs, res;
   for (auto ref : RefVec)
   {
...
std::vector<uint32_t> CDynPatcher::FindRef_Push(uint32_t StartAddr, const std::vector<uint32_t> &RefVec)
{
   std::vector<uint32_t> Refs,res;
   for (auto ref:RefVec)
   {
...
std::vector<uint32_t> CDynPatcher::FindRef_Call(uint32_t StartAddr, const std::vector<uint32_t> &RefVec)
{
   std::vector<uint32_t> Refs, res;
   for (auto ref : RefVec)
   {
...
std::vector<uint32_t> CDynPatcher::FindRef_Jmp(uint32_t StartAddr, const std::vector<uint32_t> &RefVec)
{
   std::vector<uint32_t> Refs, res;
   for (auto ref : RefVec)
   {
...
iBool CDynPatcher::AddMemChangeInfo(void *Addr, uint32_t OriginalValue, void* OriginalFuncAddr /*= NULL*/)
{
   ...
   for (auto MemData : MemChange)
   {
      if (OriginalFuncAddr)
      {
//          if (reinterpret_cast<uint32_t>(OriginalFuncAddr) == reinterpret_cast<uint32_t>(MemData.OldFuncAddr))
//          {
//             AlreadyExists = bTrue;
//             break;
//          }
         if (reinterpret_cast<uint32_t>(Addr) == reinterpret_cast<uint32_t>(MemData->pAddr))
         {
            AlreadyExists = bTrue;
            break;
         }
...
void* CDynPatcher::HookFunctionCall(void *OrigAddr, void *NewAddr)
{
   ...
   for (uint32_t ref:CallRefs)
   {
      ...
      DynMsg("Call %i to 0x%p(%s) at %x(%s)\n", NumCalls++,OrigAddr, AddrInfo.dli_sname?AddrInfo.dli_sname:"_", ref,AddrInfo.dli_sname?RefInfo.dli_sname:"_");
#else
      DynMsg("Call %i to %p at %p\n", NumCalls++, OrigAddr, ref);
#endif
      
      FuncHook2_s* hook;
      uint32_t OldProt  = MProtect(reinterpret_cast<void*>(ref), 8, PAGE_EXECUTE_READWRITE);
      if (OldProt!=PAGE_NOACCESS)
      {
         AddMemChangeInfo(reinterpret_cast<void*>(ref + 1), *reinterpret_cast<uint32_t*>(ref + 1), OrigAddr);
...
uint32_t CDynPatcher::UnsetFunctionHook(void *OrigAddr)
{
   ...
   {
      if (reinterpret_cast<uint32_t>((*Cur)->OldFuncAddr) == reinterpret_cast<uint32_t>(OrigAddr))
      {
         delete *Cur;
         MemChange.erase(std::remove(MemChange.begin(), MemChange.end(), *Cur), MemChange.end());
         HooksRemoved++;
      }
      else
      {
...
uint32_t CDynPatcher::ChangePtrValue(void *Addr, void *NewValue)
{
   ...
   while (Cur != MemChange.end())
   {
      if (reinterpret_cast<uint32_t>((*Cur)->pAddr) == reinterpret_cast<uint32_t>(Addr))
      {
...
uint32_t CDynPatcher::DumpVtable(void **VTable)
{
   ...
   while (IsValidCodeAddr(VTable[i]))
   {
      ...
      if (dladdr(VFuncAddr, &info))
      {
         ...
         //LOG_MESSAGE(PLID,"\\*%02i:*\\ \"%s\"  (%p,%i)", i, status?info.dli_sname:DName,VFuncAddr,(uint32_t)VFuncAddr-(uint32_t)info.dli_saddr);
         printf("\\*%02i:*\\ \"%s\"  (%p,%i)\n", i, status?info.dli_sname:DName,VFuncAddr,(uint32_t)VFuncAddr-(uint32_t)info.dli_saddr);
         if(!status)
         {
            free(DName);
         }
...
uint32_t CDynPatcher::HookImportFunc(const char *FuncName, void *NewAddr, const char *Library /*= NULL*/)
{
   ...
   for (auto ImpDescr : ImportDescription)
   {
      ...
      {
         if (!ImpDescr->FunctionName) {continue; }
   ...
   if (Candidates>1)
   {
      DynWarn("%i candidates for import func %s found!", Candidates, FuncName);
   }
   ...
   if (ImpData)
   {
      if (NewAddr)
      {
         ...
         {
            HookFunctionCall(reinterpret_cast<void*>(ImpData->ImpFuncAddr), NewAddr);
         }
      ...
      else
      {
         ...
         if (ImpData->hasDynRel)
         {
            UnsetFunctionHook(reinterpret_cast<void*>(ImpData->ImpFuncAddr));
         }
...
uint32_t CDynPatcher::GetImportFuncAddr(const char *FuncName, const char *Library /*= NULL*/)
{
...
void CDynPatcher::UnsetHooks()
{
   ...
   for(auto MemInfo:MemChange)
   {
      ...
      if (OldProtect)
      {
			if (OldProtect != PAGE_NOACCESS)
			{
				*reinterpret_cast<uint32_t*>(MemInfo->pAddr) = MemInfo->OrigValue;
...
void ** CDynPatcher::FindVTable(void* address, iBool MayBeInCode/*=bFalse*/)
{
   ...
   for (int i = 0; i < 0xFF; ++i)
   {
      vtAddr = *reinterpret_cast<void**>(reinterpret_cast<uint32_t>(address)+i);
		if (IsRangeInData(reinterpret_cast<uint32_t>(vtAddr), 4) || (MayBeInCode==bTrue&&IsRangeInCode(reinterpret_cast<uint32_t>(vtAddr), 4)))
      {
         ...
         if (sum > 5)
         {
           // DynMsg("Virtual table offset=%i", i);
				return reinterpret_cast<void**>(*reinterpret_cast<uint32_t*>(address)+i);
            //return i;
         }
      }
      else
...
iBool CDynPatcher::IsValidCodeAddr(void *Addr)
{
   ...
   if(Flags&PROT_EXEC)
#else
   if (Flags&PAGE_EXECUTE || Flags&PAGE_EXECUTE_READ || Flags&PAGE_EXECUTE_READWRITE || Flags&PAGE_EXECUTE_WRITECOPY)
#endif
   {
      return bTrue;
   }
